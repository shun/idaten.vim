...
- コンパイル成果物は単一の `state.vim`
- 通常起動では Deno を起動しない（`sync/compile` と必要な遅延処理時のみ使用）
- 実行時の探索は行わず、`compile` で列挙したファイルのみを `source` する
- 遅延読み込み v1 は event / FileType / command のみ対応
- 取得元は git URL のみ（https/ssh/git）。ローカルパスは dev override のショートハンドとして扱う
- class 構文は全面禁止
- Vim script は Vim9 を使わず、従来の Vim script で書く
- Vim script は最小限に留め、重い処理は denops/TypeScript に寄せる
...
### 2.5 バージョン管理
- `src/version.ts` の `IDATEN_VERSION` を唯一のソースとする
- `deno.json` に `version` は置かない
...
- import map は設定読み込み時に必要に応じて生成する
- compile 実行時は必ず再生成する
...
- FileType 時の特殊処理:
  - 該当 FileType のプラグインをロード
  - 既にロード済みの全プラグインの該当 filetype ファイルも source
...
- `g:idaten_repo_url` で idaten 本体の URL を指定可能（既定: https://github.com/shun/idaten.vim.git）
...
- ディレクトリ指定時は `<path>/idaten.log` に出力

### 7.16 コマンド補完
- `:Idaten` はサブコマンド補完を提供する
- 補完対象は `compile/sync/update/status/check/clean/lock` とする
- `:Idaten sync` では `--locked` と `--config` を補完対象に含める
- `:Idaten compile` では `--config` を補完対象に含める
- `:Idaten update` では `--rev` と `--self` を補完対象に含める

## 8. データ仕様

### 8.1 管理ディレクトリ
- プラグイン clone、state.vim、lockfile、import map を保持
...
- 詳細は [03-data-structures.md](03-data-structures.md) を参照
...
### 8.4 import_map.json
- `idaten` を `jsr:@shun/idaten-vim@<version>` または ローカルパスに解決
- 詳細は [03-data-structures.md](03-data-structures.md) を参照
...
- `g:idaten_repo_path`: idaten 本体のローカルパス（開発用）
- `g:idaten_repo_url`: idaten 本体の clone 元 URL
